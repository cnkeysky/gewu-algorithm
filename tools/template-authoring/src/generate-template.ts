 ...
 */
export const GENERIC_INSTRUCTION = `Create one AlgorithmUnit for the following problem. Infer domain, category, prerequisites, implementation variants, complexity, assumptions, tests, patterns, relationships, and the selected practice projections from the problem text; do not assume a fixed algorithm, signature, or naming beyond the requested language. problem.statement must be the complete learner-facing Markdown problem, not a summary; keep $...$, $$...$$, \\(...\\), or \\[...\\] formulas and never leak the solution. The manifest id must be a dotted lowercase identifier with at least one dot (array.two-sum). Implementation keys and language identifiers must be lowercase slugs (python-teaching, python); every shadow_typing and code_recall implementation reference must match a declared implementations[].key. Use code/python.py as the implementation source, include tests/python_test.py in test_references, and return both in sources; each implementation normalization must be line_endings "lf" and whitespace "strict". tests/python_test.py must load the implementation via importlib.util.spec_from_file_location from the unit root (never "from code.python import ...", which the standard library shadows). code_recall layouts: full_recall reconstructs the implementation; cloze and comment_guided use structured slots whose expected code appears verbatim in code/python.py (source_template is derived server-side); comment_to_code provides ordered comments. Non-"none" assistance requires a nonempty scaffold; "none" requires an empty one. provenance.sources[].role must be "primary", "synthesis", or "lead". With more than one requested variant, generate exactly that many distinct variants and bind every practice projection to a declared one. Return only the schema fields.`;
