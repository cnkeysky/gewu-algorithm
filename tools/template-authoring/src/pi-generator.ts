...
export function optionsFromEnvironment(
  ...
  const relay = isRelayProvider(provider);
  return {
    provider,
    ...
    toolChoice: parseToolChoice(environment.GEWU_LLM_TOOL_CHOICE) ?? (relay ? "auto" : "forced"),
    reasoningEffort: nonEmpty(environment.GEWU_LLM_REASONING_EFFORT) ?? (relay ? "none" : undefined),
  };
}

/**
 * Builds the Pi-ai model catalog from the provider registry: built-in
 * providers come from Pi-ai, and every `relay` entry in providers.json whose
 * `baseUrlEnv` is set registers an OpenAI-compatible provider that routes to
 * that endpoint. Relay credentials come only from the entry's `keyEnv` —
 * never from a provider key, so a real DeepSeek/OpenAI key can never be sent
 * to an arbitrary relay endpoint.
 */
...
export class PiGenerator {
  ...
  async generate(task: DraftTask): Promise<DraftArtifact> {
    ...
    if (!model && this.#isRelay) {
      const entry = providerEntry(this.#options.provider);
      throw new Error(
        `Pi-ai relay model not found: ${this.#options.provider}/${this.#options.model} — set GEWU_LLM_PROVIDER=${entry?.id ?? "relay"}, GEWU_LLM_MODEL=<relay model id>, and ${entry?.baseUrlEnv ?? "GEWU_LLM_BASE_URL"} (see providers.json)`,
      );
    }
    if (!model)
...
export function isTransientPiError(message: string | undefined): boolean {
  ...
  return /429|5\d\d|econnreset|etimedout|timeout|socket|overloaded|try again later|temporarily|stream ended|stream failed|finish_reason|api_error|no tool call found/.test(text);
}
...
export function redactSecretLikeText(value: string): string {
  return value
    // URL userinfo (https://user:password@host) is echoed by some upstreams.
