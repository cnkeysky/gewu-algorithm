...
function codeRecallLayoutInstruction(profile: GenerationProfile): string {
  ...
  const sourcePath = sourcePathFor(language);
  ...
  const cloze = profile.code_recall_layouts.includes("cloze")
    ? ` For each cloze projection, declare layout "cloze", nonempty slots with lowercase slug ids, and expected code that appears verbatim in ${sourcePath}. The server derives the source_template from those slots, so never invent expected text that is not in the implementation. Select algorithm decisions, not punctuation or arbitrary syntax.`
    : "";
  const commentGuided = profile.code_recall_layouts.includes("comment_guided")
    ? ` For each comment_guided projection, use assistance "comments", declare nonempty slots with lowercase slug ids, give every slot a concise nonempty reviewed cue describing the algorithm operation without revealing its code, and ensure every slot's expected code appears verbatim in ${sourcePath}.`
    : "";
  const commentToCode = profile.code_recall_layouts.includes("comment_to_code")
    ...
    : "";
  return `\n\nRequested Code Recall layouts: ${requested}.${cloze}${commentGuided}${commentToCode} The manifest id must be a dotted lowercase identifier with at least one dot, ending with the implementation language segment, such as graph.course-schedule.python; never use a bare hyphenated id. Use exactly ${sourcePath} as the implementation source and ${testPath} as its test_references path, and return both files in sources.`;
}
