...
| `startSession`, `applyEvent`, `stopSession`, `restartSession` | Rust-owned `shadow_typing`, `flow_recall`, and `code_recall` transitions. Restart creates a fresh session rather than mutating a terminal attempt. |
